<?php
/**
 * Moja.php v3.16 - Enterprise Micro-Framework for PHP (SleekDB, Auth, ACL, Templating)
 * ============================================================================
 * - Pretty URLs (/index.php/route), .htaccess auto-bootstrap
 * - SleekDB-style JSON database (MojaDB)
 * - Enterprise-grade User/Auth/ACL (MojaUser)
 * - Minimal templating (MojaTemplate)
...
(function() {
    ...
    // Dynamically detect the subdirectory name (e.g. 'moja', 'cms', etc.)
    ...
    $htaccessContents = <<<HT
# Moja.php .htaccess for pretty URLs and index.php redirect
...
// --- Enterprise User/Auth/ACL (MojaUser) using MojaDB ---
class MojaUser {
    ...
    public function __construct() {
        $this->db = new MojaDB(self::USER_STORE, self::USER_DATA_DIR);
        if (session_status() === PHP_SESSION_NONE) {
            session_name(self::DOMAIN . '_MojaUserSess');
            session_start();
        }
        if (!empty($_SESSION['user_id'])) {
            $this->authenticated = $this->authBySession();
        } elseif (!empty($_COOKIE[self::DOMAIN . '_MojaUserID']) && !empty($_COOKIE[self::DOMAIN . '_MojaUserToken'])) {
    ...
    public function login($user, $password, $remember = false) {
        ...
        if ($remember) {
            ...
            setcookie(self::DOMAIN . '_MojaUserID', $u['_id'], time() + self::REMEMBER_COOKIE_TIMEOUT, '/', '.' . self::DOMAIN, false, true);
            setcookie(self::DOMAIN . '_MojaUserToken', $rtoken, time() + self::REMEMBER_COOKIE_TIMEOUT, '/', '.' . self::DOMAIN, false, true);
        }
    ...
    public function logout() {
        ...
        $_SESSION = [];
        session_destroy();
        setcookie(self::DOMAIN . '_MojaUserID', '', time() - 3600, '/', '.' . self::DOMAIN, false, true);
        setcookie(self::DOMAIN . '_MojaUserToken', '', time() - 3600, '/', '.' . self::DOMAIN, false, true);
    ...
    private function authByRememberMe() {
        $id = $_COOKIE[self::DOMAIN . '_MojaUserID'] ?? null;
        $token = $_COOKIE[self::DOMAIN . '_MojaUserToken'] ?? null;
        $u = $this->db->where('_id', '=', $id)->fetch();
        if (!$id || !$token || !$u) return false;
        $user = $u[0];
...
class Moja {
    ...
    private function __construct() {
        ...
        $this->router->get('/', fn($req, $res) => Moja::view_string(
            '<h1>Welcome to Moja.php v3.16!</h1><p>Enterprise micro-framework by John "Kesh" Mahugu.</p>',
            []
        ));
    }
    ...
    public static function user() { return self::instance()->auth ??= new MojaUser(); }
...
// ... (rest of your Moja.php framework code above)

/**
SAMPLE WORKING APP

// Save as index.php and place in the same folder as /moja.php
require_once __DIR__.'/moja.php';

// Home route
Moja::get('/', function($req, $res) {
    $user = Moja::user();
    $msg = $user->isAuthenticated()
        ? "Welcome, <b>{$user->getUsername()}</b>! Your role: <b>{$user->getRoleName()}</b>"
        : "Welcome, guest! Please log in or register.";
    return Moja::view_string("
        <h1>Moja Microframework v3.16 Demo</h1>
        <p>$msg</p>
        <p>
            <a href='" . moja_url('register') . "'>Register</a> | 
            <a href='" . moja_url('login') . "'>Login</a> | 
            <a href='" . moja_url('logout') . "'>Logout</a>
        </p>
    ");
});

// Registration form
Moja::get('/register', function($req, $res) {
    return Moja::view_string('
        <h2>Register</h2>
        <form method="post" action="' . moja_url('register') . '">
            <input name="username" placeholder="Username" required>
            <input name="email" type="email" placeholder="Email" required>
            <input name="password" type="password" placeholder="Password" required>
            <button type="submit">Register</button>
        </form>
        <p><a href="' . moja_url() . '">Home</a></p>
    ');
});
Moja::post('/register', function($req, $res) {
    $user = Moja::user();
    $ok = $user->register(
        $req->input('username'),
        $req->input('email'),
    ...
    return Moja::view_string($ok
        ? "<b>Registration successful.</b> <a href='" . moja_url('login') . "'>Login</a>"
        : "<b>Registration failed:</b> ".implode('; ', $user->getErrors())
    );
});

// Login form
Moja::get('/login', function($req, $res) {
    return Moja::view_string('
        <h2>Login</h2>
        <form method="post" action="' . moja_url('login') . '">
...
Moja::post('/login', function($req, $res) {
    $user = Moja::user();
    $ok = $user->login(
        $req->input('user'),
        $req->input('password'),
    ...
    return Moja::view_string($ok
        ? "<b>Login successful.</b> <a href='" . moja_url() . "'>Go Home</a>"
        : "<b>Login failed:</b> ".implode('; ', $user->getErrors())
    );
});

// Logout
Moja::get('/logout', function($req, $res) {
    $user = Moja::user();
    $user->logout();
    return Moja::view_string("You have been logged out. <a href='" . moja_url() . "'>Return home</a>");
});

// Profile (protected)
Moja::get('/profile', function($req, $res) {
    $user = Moja::user();
    if (!$user->isAuthenticated()) {
        return $res->status(403)->body = "<b>Please <a href='" . moja_url('login') . "'>login</a> to view your profile.</b>";
    }
    return Moja::view_string("
        <h2>Your Profile</h2>
...
// Admin-only page
Moja::get('/admin', function($req, $res) {
    $user = Moja::user();
    if (!$user->isAdmin()) {
...
// JSON API route
Moja::get('/api/user', function($req, $res) {
    $user = Moja::user();
    if (!$user->isAuthenticated()) {
...
// Simple notes CRUD using MojaDB
Moja::get('/notes', function($req, $res) {
    $db = Moja::db('notes');
    $notes = $db->fetch();
    $out = "<h2>Notes</h2><ul>";
    foreach ($notes as $n) {
    ...
    $out .= "</ul>
        <form method='post' action='" . moja_url('notes') . "'>
            <input name='text' placeholder='New note...' required>
            <button>Add</button>
        </form>
        <p><a href='" . moja_url() . "'>Home</a></p>";
    return Moja::view_string($out);
});
Moja::post('/notes', function($req, $res) {
    $db = Moja::db('notes');
    if (trim($req->input('text'))) {
        $db->insert(['text' => $req->input('text'), 'created' => time()]);
    }
    header('Location: ' . moja_url('notes'));
    exit;
});
